...
namespace :deploy do
  ...
  task :create_symlink do
    on roles(:app) do
        ...
        info "If there is a symbolic link to 'site/mosaic', delete it. Irregardless, create a new symbolic link to 'site/mosaic'."
        execute "cd #{fetch(:live_root)} && if [ -h site/mosaic ]; then rm site/mosaic; fi && ln -sf #{current_path} ./site/mosaic"
